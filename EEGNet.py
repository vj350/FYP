...
def run_eegnet_cv(
    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        print(f"EEGNet Fold {fold} accuracy: {acc:.4f}")
        print(f"EEGNet Fold {fold} training time: {train_time:.4f} s\n")
...
if __name__ == "__main__":
    ...
    print("Running EEGNet experiment with config:", config)
    ...
    eegnet_accuracies, eegnet_times = run_eegnet_cv(
        X,
        y,
        groups,
        config=config,
    ...
    print("\n===== EEGNet Final Results =====")
    print(f"Mean accuracy: {np.mean(eegnet_accuracies):.4f}")
    print(f"Std accuracy : {np.std(eegnet_accuracies):.4f}")
    print(f"Avg training time: {np.mean(eegnet_times):.4f} s")